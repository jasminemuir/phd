...
def doOpening(iarray, maxWindowSize, winSize1, c, s, dh0, dhmax):
    ...
    for wk in winSize1:
        ...
        if wk <= maxWindowSize:
            ...
            #threshold or the zalue is less than the inumpyut
...
def applyPMF(dataArr, noDataMask, binGeoSize, zNull, initWinSize=1, maxWinSize=10, winSizeInc=1, slope=0.3, dh0=0.3, dhmax=5, expWinSizes=False):
    """
    Apply the progressive morphology filter (PMF) to the inumpyut data array (dataArr) 
    ...
    * noDataMask is a numpy array specifying the cells in the inumpyut dataArr which do not
    ...
    * expWinSizes is a boolean specifying whether the windows sizes should increase exponentially or not (Default = False)
    ...
    Returns:
    PMF Filtered array with the same data type as the inumpyut.   
    ...
    A = dataArr.astype(numpy.float32)
    
    (nCols, nRows) = A.shape
    
    # Use nearest neighbour to interpolate cells where there is a noData value
    nn = doNearestNeighbourInterp(A, noDataMask, nCols, nRows)
    ...
    opening = doOpening(nn, maxWinSize, winSize_tminus1, binGeoSize, slope, dh0, dhmax)
    print("opening",opening.max())
    

    # Find the difference between the output of the opening (A) and the copy of original array (B)
    diff = numpy.absolute(A-opening)
    ...
    C = numpy.where(diff<=0.2, A, zNull)
    
    #C = numpy.where(B<=0.2, 2, 0)
