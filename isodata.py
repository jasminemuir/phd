...
def initial_clusters(img_flat, k, method="linspace"):
    ...
    if method == "linspace":
        ...
        ##modified to make centers not include min/max (as was in orig code)
        centers_temp = np.linspace(min, max, k)
        dis_temp = (centers_temp[1]-centers_temp[0])/2.0
        mina = min + dis_temp
        maxa = max - dis_temp
        centers = np.linspace(mina, maxa, k)
    elif method == "random":
...
def isodata_classification(img, parameters=None):
    ...
    for iter in xrange(0, I):
        ...
        ##discard clusters if number of pixels is less than THETA_M
        centers, clusters_list = discard_clusters(img_class_flat,
                                                  centers, clusters_list)
        print(centers, "after discard")
        #sys.exit()
...
###############################################################################
    ...
    return img_class_flat.reshape(N, M), k
